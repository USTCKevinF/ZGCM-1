    ...
    def transformer_flops():
        ...
        if args.attention_output_gate:
            ...
            if getattr(args, 'attention_output_gate_only_swa', False):
                standard_layer_numbers = range(1, num_layers + 1)
                if num_linear_attention_layers:
                    standard_layer_numbers = [
                        i + 1
                        for i, is_linear in enumerate(linear_attention_pattern)
                        if not is_linear
                    ]
                num_attention_output_gate_layers = sum(
                    is_layer_window_attention(
                        args.window_size,
                        args.window_attn_skip_freq,
                        layer_number,
                    )
                    for layer_number in standard_layer_numbers
                )
