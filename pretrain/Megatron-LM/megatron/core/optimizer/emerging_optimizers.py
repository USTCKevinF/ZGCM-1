...
def _get_qkv_split_shapes(model_cfg) -> List[List[int]]:
    """Compute candidate QKV split shapes from model config.

    When attention output gating is enabled on only a subset of layers, Muon
    must handle both fused projection layouts:
    - Q/K/V
    - Q/G/K/V
    """
    query_hidden = (
    ...
    ungated = [query_hidden, model_cfg.kv_channels, model_cfg.kv_channels]
    if not getattr(model_cfg, "attention_output_gate", False):
        return [ungated]

    gated = [query_hidden, query_hidden, model_cfg.kv_channels, model_cfg.kv_channels]
    if getattr(model_cfg, "attention_output_gate_only_swa", False) or getattr(
        model_cfg, "attention_output_gate_only_global", False
    ):
        return [gated, ungated]
    return [gated]


def _get_num_query_groups_per_partition(model_cfg, tp_world_size: int) -> int:
    """Infer the per-partition query-group count for fused QKV weights."""
    if model_cfg.num_query_groups < tp_world_size:
        return 1
    return model_cfg.num_query_groups // tp_world_size
...
class TensorParallelMuon(OrthogonalizedOptimizer):
    ...
    def __init__(
        ...
        qkv_split_shapes: list[list[int]] | tuple[int, ...] | None = None,
        qkv_num_groups_per_partition: int | None = None,
        ...
        self.qkv_num_groups_per_partition = qkv_num_groups_per_partition
    ...
    def orthogonalize(self, p: torch.Tensor, grad: torch.Tensor, **kwargs: Any) -> torch.Tensor:
        ...
        if self.split_qkv and self.is_qkv_fn(p):  # type: ignore[misc]
            ...
            if self.qkv_split_shapes is None:
                raise RuntimeError(
                    "split_qkv is enabled for Muon, but qkv_split_shapes is not configured."
                )
            split_shape_options = self.qkv_split_shapes
            if isinstance(split_shape_options[0], int):
                split_shape_options = [split_shape_options]

            selected_split_shapes = None
            num_query_groups = None
            for split_shapes in split_shape_options:
                split_width = sum(split_shapes)
                if self.qkv_num_groups_per_partition is not None:
                    if grad_shape[0] == self.qkv_num_groups_per_partition * split_width:
                        selected_split_shapes = split_shapes
                        num_query_groups = self.qkv_num_groups_per_partition
                        break
                elif grad_shape[0] % split_width == 0:
                    selected_split_shapes = split_shapes
                    num_query_groups = grad_shape[0] // split_width
                    break

            if selected_split_shapes is None or num_query_groups is None:
                raise RuntimeError(
                    "Muon QKV split shapes do not match the fused attention projection. "
                    f"grad_shape={grad_shape}, split_shape_options={self.qkv_split_shapes}, "
                    f"expected_num_query_groups={self.qkv_num_groups_per_partition}"
                )

            qkv_grads = torch.split(
                grad.view(num_query_groups, sum(selected_split_shapes), -1),
                selected_split_shapes,
                dim=1,
            )
...
class TensorParallelAdaptiveMuon(TensorParallelMuon, AdaptiveMuon):
    ...
    def __init__(
        ...
        TensorParallelMuon.__init__(
            ...
            qkv_num_groups_per_partition=qkv_num_groups_per_partition,
            fp32_matmul_prec=fp32_matmul_prec,
            coefficient_type=coefficient_type,
...
def _muon_config_to_kwargs(config, model_chunks, pg_collection) -> Dict[str, Any]:
    ...
    kwargs["qkv_num_groups_per_partition"] = _get_num_query_groups_per_partition(
        model_chunks[0].config, get_pg_size(pg_collection.tp)
    )
