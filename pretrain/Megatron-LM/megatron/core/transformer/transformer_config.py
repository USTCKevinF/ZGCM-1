...
class TransformerConfig(ModelParallelConfig):
    ...
    """Whether to apply the attention output gate only to sliding-window layers."""
    ...
    def __post_init__(self):
        ...
        if self.attention_output_gate_only_swa and not self.attention_output_gate:
            raise ValueError(
                "attention_output_gate_only_swa requires attention_output_gate=True."
            )
        if self.attention_output_gate_only_swa and self.window_size is None:
            raise ValueError(
                "attention_output_gate_only_swa requires sliding-window attention."
            )
