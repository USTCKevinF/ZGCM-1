...
_DCP_GLOBAL_GLOO_GROUP = None


def _get_dcp_global_gloo_group():
    """Use a full-world Gloo group for PyTorch DCP object collectives during load."""
    global _DCP_GLOBAL_GLOO_GROUP
    if not dist.is_available() or not dist.is_initialized():
        return None
    if _DCP_GLOBAL_GLOO_GROUP is None:
        _DCP_GLOBAL_GLOO_GROUP = dist.new_group(backend="gloo")
    return _DCP_GLOBAL_GLOO_GROUP
...
class TorchDistLoadShardedStrategy:
    ...
    def load(
        ...
        checkpoint.load_state_dict(
            pyt_state_dict,
            fsr,
            process_group=_get_dcp_global_gloo_group(),
