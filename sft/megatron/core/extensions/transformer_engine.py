    ...
    def forward(
        ...
        if self.te_forward_mask_type:
            if qkv_format == "thd" and is_te_min_version("1.7.0"):
                ...
                # Transformer Engine's A2A context-parallel path asserts that padding-style masks
                # are unsupported, while SFT packed THD inputs still need causal masking. Keep the
                # original causal/no_mask type when a CP group is active.
                cp_comm_type = self.cp_comm_type
                use_cp_a2a = cp_comm_type is not None and "a2a" in str(cp_comm_type)
                if attn_mask_type == AttnMaskType.causal and not use_cp_a2a:
                    attn_mask_type = AttnMaskType.padding_causal
                elif attn_mask_type == AttnMaskType.no_mask and not use_cp_a2a:
