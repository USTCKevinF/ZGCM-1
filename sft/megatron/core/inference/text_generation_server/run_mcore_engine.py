...
def run_mcore_engine(
    ...
    if mpu.is_pipeline_first_stage():
        response_dict = {
            ...
            "tokens": [_as_list(x.prompt_tokens) + _as_list(x.generated_tokens) for x in result],
        }
        if sampling_params.return_log_probs:
            response_logprobs = [
                _as_list(x.prompt_log_probs) + _as_list(x.generated_log_probs) for x in result
            ]
        ...
        if sampling_params.top_n_logprobs > 0:
            ...
            response_dict["top_n_logprobs"] = [
                _as_list(x.prompt_top_n_logprobs) + _as_list(x.generated_top_n_logprobs)
                for x in result
            ]
