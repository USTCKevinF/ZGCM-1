...
class LanguageModule(MegatronModule):
    ...
    def _set_attention_backend(self):
        ...
        effective_attention_backend = self.config.attention_backend
        disable_fused_attention = False
        if (
            self.config.attention_sink_bias
            and effective_attention_backend == AttnBackend.flash
        ):
            logging.warning(
                "attention_sink_bias uses Transformer Engine post_scale_bias, which is not "
                "supported by FlashAttention. Falling back to attention-backend=auto so "
                "Fused/Unfused TE backends stay available on sink-bias layers."
            )
            effective_attention_backend = AttnBackend.auto
        if (
            self.config.softmax_type != "vanilla"
            and effective_attention_backend == AttnBackend.flash
        ):
            logging.warning(
                "softmax_type=%s is not reliably supported by FlashAttention on our TE stack. "
                "Falling back to attention-backend=auto so TE can choose a compatible backend.",
                self.config.softmax_type,
            )
            effective_attention_backend = AttnBackend.auto
        if self.config.attention_sink_bias and effective_attention_backend == AttnBackend.auto:
            # cuDNN fused attention is either unsupported for SWA post_scale_bias or unstable for
            # learnable post_scale_bias backward on full-attention layers in our stack.
            # Keep FlashAttention available for no-bias layers and route sink-bias layers to the
            # TE Unfused backend instead.
            disable_fused_attention = True

        if effective_attention_backend == AttnBackend.local:
        ...
        elif effective_attention_backend == AttnBackend.flash:
            check_and_set_env_variable("NVTE_FLASH_ATTN", 1, AttnBackend.flash)
            check_and_set_env_variable("NVTE_FUSED_ATTN", 0, AttnBackend.flash)
            check_and_set_env_variable("NVTE_UNFUSED_ATTN", 0, AttnBackend.flash)
        elif effective_attention_backend == AttnBackend.fused:
        ...
        elif effective_attention_backend == AttnBackend.unfused:
        ...
        elif effective_attention_backend == AttnBackend.auto:
            check_and_set_env_variable("NVTE_FLASH_ATTN", 1, AttnBackend.auto)
            check_and_set_env_variable(
                "NVTE_FUSED_ATTN", 0 if disable_fused_attention else 1, AttnBackend.auto
            )
