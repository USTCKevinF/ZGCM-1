...
def _fb_detail_bool_env(name, default=False):
...
def _fb_detail_int_env(name, default):
    value = os.environ.get(name)
    if value is None:
        return default
    try:
...
def _fb_detail_enabled(call_id):
    if not _fb_detail_bool_env("MEGATRON_FB_DETAIL_TIMING", False):
        return False
    interval = max(1, _fb_detail_int_env("MEGATRON_FB_DETAIL_TIMING_INTERVAL", 10))
    return call_id % interval == 0


def _fb_detail_now(enabled):
    if enabled and _fb_detail_bool_env("MEGATRON_FB_DETAIL_TIMING_CUDA_SYNC", True):
        torch.cuda.synchronize()
    return time.perf_counter()


def _fb_detail_record(phases, name, start, enabled):
    if not enabled:
        return start
    now = _fb_detail_now(enabled)
    phases[name] = phases.get(name, 0.0) + now - start
    return now


def _fb_detail_report(call_id, num_microbatches, phases):
    ...
    for index, name in enumerate(names):
        ...
        print(
            "[fb-detail-timing] "
            f"call={call_id} microbatches={num_microbatches} phase={name} "
            f"min_ms={float(torch.min(phase).item()):.3f} "
            f"mean_ms={float(torch.mean(phase).item()):.3f} "
            f"p95_ms={float(torch.quantile(phase, 0.95).item()):.3f} "
...
def forward_backward_no_pipelining(
    *,
    ...
    global _FB_DETAIL_CALL
    _FB_DETAIL_CALL += 1
    fb_detail_call = _FB_DETAIL_CALL
    fb_detail = _fb_detail_enabled(fb_detail_call)
    fb_detail_phases = {}
    ...
    if config.overlap_moe_expert_parallel_comm and not forward_only:
        fb_detail_start = _fb_detail_now(fb_detail)
        ...
        _fb_detail_record(fb_detail_phases, "schedule_body", fb_detail_start, fb_detail)
        ...
        with no_sync_func():
            for i in range(num_microbatches - 1):
                ...
                fb_detail_start = _fb_detail_record(
                    fb_detail_phases, "micro_forward", fb_detail_start, fb_detail
                )
                total_num_tokens += num_tokens
                if not forward_only:
                    backward_step(input_tensor, output_tensor, output_tensor_grad, config)
                    _fb_detail_record(
                        fb_detail_phases, "micro_backward_no_sync", fb_detail_start, fb_detail
                    )
        ...
        if not forward_only:
            backward_step(input_tensor, output_tensor, output_tensor_grad, config)
            _fb_detail_record(
                fb_detail_phases, "micro_backward_sync", fb_detail_start, fb_detail
            )
    ...
    if config.finalize_model_grads_func is not None and not forward_only:
        ...
        _fb_detail_record(
            fb_detail_phases, "finalize_model_grads", fb_detail_start, fb_detail
        )
    ...
    if getattr(config, 'fine_grained_activation_offloading', False):
        ...
        off_interface.reset()
        _fb_detail_record(
            fb_detail_phases, "activation_offload_reset", fb_detail_start, fb_detail
        )
    ...
    if fb_detail:
        fb_detail_phases["forward_backward_total"] = sum(fb_detail_phases.values())
        _fb_detail_report(fb_detail_call, num_microbatches, fb_detail_phases)
