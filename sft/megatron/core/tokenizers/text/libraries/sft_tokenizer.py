...
class SFTTokenizer:
    ...
    def __init__(self, tokenizer_path: str, prompt_format: str):
        ...
        if HAVE_TRANSFORMERS:
            # Currently, only HuggingFace tokenizers are supported. Some local
            # GLM-5.1 snapshots advertise a non-Transformers tokenizer class
            # ("TokenizersBackend") while still shipping a standard
            # tokenizer.json; fall back to PreTrainedTokenizerFast for those.
            try:
                ...
                if "TokenizersBackend" not in str(exc) or not tokenizer_file.exists():
                    raise
        ...
        if getattr(tokenizer, "chat_template", None) is None and template_path.exists():
        ...
        elif prompt_format == "glm51":
            if tokenizer.chat_template is None:
                raise ValueError(
                    "glm51 SFT prompt format requires chat_template.jinja or tokenizer.chat_template"
                )
            self._prompt_config = PromptConfig(
                assistant_prefix_len=0,
                pad_token_id=(
    ...
    def _apply_chat_template_tokens(
        self, conversation: List[Dict], add_generation_prompt: bool, enable_thinking: bool = False
    ):
        def _to_token_array(value):
            if isinstance(value, dict) and "input_ids" in value:
                return _to_token_array(value["input_ids"])
            if hasattr(value, "data") and "input_ids" in value:
                return _to_token_array(value["input_ids"])
            if hasattr(value, "ids"):
                return np.asarray(value.ids, dtype=np.int64)
            if isinstance(value, np.ndarray):
                if value.dtype == object and value.size == 1:
                    return _to_token_array(value.reshape(-1)[0])
                return value[0] if value.ndim > 1 else value
            if isinstance(value, (list, tuple)):
                if len(value) == 1:
                    return _to_token_array(value[0])
        ...
        return _to_token_array(tokens)

    def _tokenize_conversation_glm51(
        self, conversation: List[Dict], return_target: bool, add_generation_prompt: bool
    ):
        tokens = self._apply_chat_template_tokens(
            conversation, add_generation_prompt=add_generation_prompt, enable_thinking=False
        )
        ...
        for turn_idx, turn in enumerate(conversation):
            if turn["role"].lower() != "assistant":
                continue
            if turn_idx == 0 or conversation[turn_idx - 1]["role"].lower() not in ("user", "tool"):
                raise ValueError(f"assistant turn must follow user/tool in conversation: {conversation}.")
            content = turn.get("content", "")
            has_reasoning = (
                isinstance(turn.get("reasoning_content", None), str)
                or (isinstance(content, str) and "</think>" in content)
            )
            prefix = self._apply_chat_template_tokens(
                conversation[:turn_idx],
                add_generation_prompt=True,
                enable_thinking=has_reasoning,
            )
            through_turn = self._apply_chat_template_tokens(
                conversation[: turn_idx + 1],
                add_generation_prompt=False,
                enable_thinking=False,
            )
            start = len(prefix)
            end = len(through_turn)
            if end > len(tokens) or start > end:
                raise ValueError(f"bad GLM-5.1 assistant span {start}:{end} for {conversation}.")
    ...
    def tokenize_conversation(
        ...
        if self._prompt_format == "glm51":
            return self._tokenize_conversation_glm51(
                conversation, return_target=return_target, add_generation_prompt=add_generation_prompt
            )
