...
class Attention(MegatronModule, ABC):
    ...
    def __init__(
        ...
        self.is_window_attention_layer = False
        self.use_attention_output_gate = False
        if self.attention_type == "self":
            self.is_window_attention_layer = is_layer_window_attention(
                self.config.window_size, self.config.window_attn_skip_freq, self.layer_number
            )
            if self.config.attention_output_gate:
                apply_on_this_layer = True
                if self.config.attention_output_gate_only_swa:
                    apply_on_this_layer = self.is_window_attention_layer
                elif self.config.attention_output_gate_only_global:
                    apply_on_this_layer = not self.is_window_attention_layer
                self.use_attention_output_gate = apply_on_this_layer

        self.use_attention_sink_bias = False
        self.attention_sink_bias_weight = None
        if self.config.attention_sink_bias and self.attention_type == "self":
            core_attention_name = type(self.core_attention).__name__
            if core_attention_name != "TEDotProductAttention":
                raise NotImplementedError(
                    "attention_sink_bias currently only supports TEDotProductAttention, "
                    f"but got {core_attention_name} on layer {self.layer_number}."
                )
            # Register the parameter on every self-attention layer so dist-checkpoint
            # sharding remains dense across the transformer stack even when the sink
            # bias is only applied on a subset of layers.
            param_device = torch.cuda.current_device() if torch.cuda.is_available() else None
            self.attention_sink_bias_weight = torch.nn.Parameter(
                torch.full(
                    (
                        self.num_attention_heads_per_partition,
                        self.config.attention_sink_bias_num_tokens,
                    ),
                    fill_value=self.config.attention_sink_bias_init,
                    dtype=self.config.params_dtype,
                    device=param_device,
                )
            )
            apply_on_this_layer = True
            if self.config.attention_sink_bias_only_swa:
                apply_on_this_layer = self.is_window_attention_layer
            elif self.config.attention_sink_bias_only_global:
                apply_on_this_layer = not self.is_window_attention_layer
            self.attention_sink_bias_weight.requires_grad_(apply_on_this_layer)
            if apply_on_this_layer:
                self.use_attention_sink_bias = True
    ...
    def _build_attention_sink_bias(
        self,
        key: Tensor,
        packed_seq_params: Optional[PackedSeqParams],
        inference_context: Optional[BaseInferenceContext],
    ) -> Optional[Tensor]:
        if not self.use_attention_sink_bias:
            return None

        if packed_seq_params is not None:
            raise NotImplementedError(
                "attention_sink_bias currently only supports unpacked sequences on the TE path."
            )

        if inference_context is not None and not inference_context.is_static_batching():
            raise NotImplementedError(
                "attention_sink_bias currently only supports training/static batching."
            )
        ...
        num_sink_tokens = min(self.config.attention_sink_bias_num_tokens, key_seq_len)
        sink_bias = key.new_zeros((1, self.num_attention_heads_per_partition, 1, key_seq_len))
        sink_values = self.attention_sink_bias_weight[:, :num_sink_tokens].to(
            device=key.device, dtype=key.dtype
        )
        sink_bias[:, :, :, :num_sink_tokens] = sink_values.unsqueeze(0).unsqueeze(2)
        return sink_bias

    def _merge_attention_sink_bias(
        self,
        attention_bias: Optional[Tensor],
        key: Tensor,
        packed_seq_params: Optional[PackedSeqParams],
        ...
        sink_bias = self._build_attention_sink_bias(key, packed_seq_params, inference_context)
        if sink_bias is None:
            return attention_bias
        if attention_bias is None:
            return sink_bias
        return attention_bias + sink_bias.to(device=attention_bias.device, dtype=attention_bias.dtype)
    ...
    def _checkpointed_attention_forward(
        ...
        **extra_kwargs,
        ...
        extra_arg_names = tuple(extra_kwargs.keys())
        ...
        def custom_forward(*inputs):
            ...
            runtime_extra_kwargs = {
                name: inputs[6 + idx] for idx, name in enumerate(extra_arg_names)
            }
        ...
        hidden_states = tensor_parallel.checkpoint(
            ...
            rotary_pos_emb,
            attn_mask_type,
            *(extra_kwargs[name] for name in extra_arg_names),
        )
        ...
        *,
        ...
        with off_interface(self.offload_qkv_linear, hidden_states, "qkv_linear") as hidden_states:
            qkv_output = self.get_query_key_value_tensors(
                ...
                key_value_states,
                split_qkv=split_qkv,
                output_gate=self.use_attention_output_gate,
            )
        ...
        if split_qkv:
            ...
            assert (
                not self.use_attention_output_gate
        ...
        attention_bias = self._merge_attention_sink_bias(
            attention_bias, key, packed_seq_params, inference_context
        )
        checkpoint_extra_kwargs = {}
        if self.config.experimental_attention_variant == "dsa":
            # Standalone DSA reuses the current hidden states as both the indexer
            # source activations and the query-side projection input.
            checkpoint_extra_kwargs["x"] = hidden_states
            checkpoint_extra_kwargs["qr"] = hidden_states
