...
class DSAIndexer(MegatronModule):
    ...
    def __init__(
        ...
        self.qk_pos_emb_head_dim = min(
            getattr(self.config, "qk_pos_emb_head_dim", self.config.kv_channels),
            self.config.dsa_indexer_head_dim,
        )
        self.q_lora_rank = (
            getattr(self.config, "q_lora_rank", None)
        ...
        self.rotary_base = getattr(self.config, "rotary_base", 10000)
        self.rotary_percent = getattr(self.config, "rotary_percent", 1.0)
        self.rotary_scaling_factor = getattr(self.config, "rotary_scaling_factor", 40)
        self.original_max_position_embeddings = getattr(
            self.config, "original_max_position_embeddings", 4096
        )
        self.beta_fast = getattr(self.config, "beta_fast", 32)
        self.beta_slow = getattr(self.config, "beta_slow", 1)
        self.mscale = getattr(self.config, "mscale", 1.0)
        self.mscale_all_dim = getattr(self.config, "mscale_all_dim", 0.0)
        ...
        if self.rope_type == 'rope':
            ...
            self.rotary_pos_emb = YarnRotaryEmbedding(
                self.qk_pos_emb_head_dim,
                rotary_base=self.rotary_base,
                scaling_factor=self.rotary_scaling_factor,
