...
class TransformerBlock(GraphableMegatronModule, MegatronModule):
    ...
    def sharded_state_dict(
        ...
        # Gate placement can make self-attention QKV weights heterogeneous across
        # layers: gated layers append an extra projection to linear_qkv, ungated
        # layers do not. Store such layers under per-layer checkpoint keys.
        if (
            self.config.attention_output_gate
            and (
                self.config.attention_output_gate_only_swa
                or self.config.attention_output_gate_only_global
            )
        ):
            non_homogeneous_layers = True
