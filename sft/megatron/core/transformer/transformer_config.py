...
class TransformerConfig(ModelParallelConfig):
    ...
    """Enable a learnable additive attention sink bias on the TE attention path."""

    attention_sink_bias_num_tokens: int = 1
    """Number of leading key positions to treat as sink tokens when attention_sink_bias is enabled."""

    attention_sink_bias_init: float = 1.0
    """Initialization value for the learnable attention sink bias logits."""

    attention_sink_bias_only_swa: bool = False
    """If True, only apply attention sink bias on sliding-window attention layers."""

    attention_sink_bias_only_global: bool = False
    """If True, only apply attention sink bias on non-sliding global attention layers."""
    ...
    attention_output_gate_only_swa: bool = False
    """Whether to apply output gate only on sliding-window attention layers."""

    attention_output_gate_only_global: bool = False
    """Whether to apply output gate only on global/full-attention layers."""
    ...
    def __post_init__(self):
        ...
        if self.attention_sink_bias_num_tokens < 1:
            raise ValueError(
                "attention_sink_bias_num_tokens must be >= 1 when attention sink bias is used."
            )

        if self.attention_sink_bias and self.attention_backend == AttnBackend.local:
            raise ValueError(
                "attention_sink_bias currently only supports the Transformer Engine attention path."
            )

        if self.attention_sink_bias and self.context_parallel_size != 1:
            raise ValueError(
                "attention_sink_bias currently only supports context_parallel_size == 1."
            )

        if self.attention_sink_bias_only_swa and self.attention_sink_bias_only_global:
            raise ValueError(
                "attention_sink_bias_only_swa and attention_sink_bias_only_global "
        ...
        if self.attention_output_gate_only_swa and self.attention_output_gate_only_global:
            raise ValueError(
                "attention_output_gate_only_swa and attention_output_gate_only_global "
                "cannot both be enabled."
            )
        if (
            self.attention_output_gate_only_swa or self.attention_output_gate_only_global
        ) and not self.attention_output_gate:
            raise ValueError(
                "attention_output_gate_only_swa/attention_output_gate_only_global require "
                "attention_output_gate to be enabled."
            )
