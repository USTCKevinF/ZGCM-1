...
def validate_args(args, defaults={}):
    ...
    if args.optimizer not in ('sgd', 'adam'):
        ...
        # Allow explicit opt-out for debugging and benchmark runs where layer-wise
        # distribution is not supported with the current TP configuration.
        if os.environ.get("MEGATRON_DISABLE_LAYERWISE_DISTRIBUTED_OPTIMIZER", "0") == "1":
            args.use_layer_wise_distributed_optimizer = False
            args.use_distributed_optimizer = False
            warn_rank_0('Disabling layer-wise distributed optimizer via '
                        'MEGATRON_DISABLE_LAYERWISE_DISTRIBUTED_OPTIMIZER=1.')
