...
def build_pretraining_data_loader(dataset, consumed_samples):
    ...
    if dataset is None:
        return None
    args = get_args()
    sample_offset = int(os.getenv("MEGATRON_DATASET_SAMPLE_OFFSET", "0"))
    if sample_offset:
        consumed_samples = max(0, consumed_samples - sample_offset)
    ...
    if (
        ...
        and os.getenv("MEGATRON_DATALOAD_CP_SOURCE_ONLY_WORKERS", "0") == "1"
    ...
    if args.hybrid_context_parallel:
        ...
        extra_kwargs = {}
    if num_workers > 0 and os.getenv("MEGATRON_DATALOADER_PREFETCH_FACTOR"):
        extra_kwargs["prefetch_factor"] = int(os.getenv("MEGATRON_DATALOADER_PREFETCH_FACTOR"))
