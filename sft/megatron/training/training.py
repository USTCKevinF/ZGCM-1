...
def num_floating_point_operations(args, batch_size):
    ...
    def transformer_flops():
        ...
        def count_window_attention_layers(total_layers):
            if not getattr(args, "window_size", None):
                return 0
            window_attn_skip_freq = getattr(args, "window_attn_skip_freq", None)
            ...
            if isinstance(window_attn_skip_freq, list):
                return sum(1 for enabled in window_attn_skip_freq[:total_layers] if enabled)
            raise ValueError(
                f"Invalid window_attn_skip_freq: "
                f"{type(window_attn_skip_freq)}, {window_attn_skip_freq}"
            )

        gate_projection_term = 0
        ...
        if args.multi_latent_attention:
            ...
            gate_projection_term = (
                forward_backward_expansion_factor
                * fma_expansion_factor
                * (args.hidden_size * query_projection_size)
            )
        ...
        num_attention_output_gate_layers = 0
        if getattr(args, "attention_output_gate", False):
            num_attention_output_gate_layers = num_standard_attention_layers
            if getattr(args, "attention_output_gate_only_swa", False):
                num_attention_output_gate_layers = count_window_attention_layers(
                    num_standard_attention_layers
                )
            elif getattr(args, "attention_output_gate_only_global", False):
                num_attention_output_gate_layers = (
                    num_standard_attention_layers
                    - count_window_attention_layers(num_standard_attention_layers)
                )

        self_attn_term = (
            linear_self_attn_term * num_linear_attention_layers
            + standard_self_attn_term * num_standard_attention_layers
            + gate_projection_term * num_attention_output_gate_layers
        )
...
def _phase_timing_bool_env(name, default=False):
...
def _phase_timing_int_env(name, default):
    value = os.environ.get(name)
    if value is None:
        return default
    try:
...
def _phase_timing_enabled(iteration):
    if iteration is None or not _phase_timing_bool_env("MEGATRON_PHASE_TIMING", False):
        return False
    interval = max(1, _phase_timing_int_env("MEGATRON_PHASE_TIMING_INTERVAL", 10))
    return (iteration + 1) % interval == 0


def _phase_timing_now(enabled):
    if enabled and _phase_timing_bool_env("MEGATRON_PHASE_TIMING_CUDA_SYNC", True):
        torch.cuda.synchronize()
    return time.perf_counter()


def _phase_timing_record(phases, name, start, enabled):
    if not enabled:
        return start
    now = _phase_timing_now(enabled)
    phases[name] = phases.get(name, 0.0) + now - start
    return now


def _phase_timing_report(iteration, phases):
    if not phases or not torch.distributed.is_initialized():
        return
    names = list(phases)
    local = torch.tensor(
        [phases[name] * 1000.0 for name in names],
        dtype=torch.float32,
    ...
    if rank != world_size - 1:
        return
    values = torch.stack(gathered, dim=0).cpu()
    for index, name in enumerate(names):
        phase = values[:, index]
        max_rank = int(torch.argmax(phase).item())
        p95 = float(torch.quantile(phase, 0.95).item())
        print(
            "[phase-timing] "
            f"iter={iteration + 1} phase={name} "
            f"min_ms={float(torch.min(phase).item()):.3f} "
            f"mean_ms={float(torch.mean(phase).item()):.3f} "
            f"p95_ms={p95:.3f} "
            f"max_ms={float(torch.max(phase).item()):.3f} "
            f"max_rank={max_rank}",
            flush=True,
        )


def _periodic_empty_cache_enabled(iteration):
    if iteration is None:
        return False
    interval = _phase_timing_int_env("MEGATRON_PERIODIC_EMPTY_CACHE_INTERVAL", 0)
    return interval > 0 and (iteration + 1) % interval == 0


def _periodic_empty_cache(iteration):
    if not _periodic_empty_cache_enabled(iteration):
        return
    torch.cuda.empty_cache()
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        print(
            f"[periodic-empty-cache] iter={iteration + 1} "
            f"interval={_phase_timing_int_env('MEGATRON_PERIODIC_EMPTY_CACHE_INTERVAL', 0)}",
            flush=True,
        )


def _periodic_free_overlap_buffers_enabled(iteration):
    if iteration is None:
        return False
    interval = _phase_timing_int_env("MEGATRON_PERIODIC_FREE_OVERLAP_BUFFERS_INTERVAL", 0)
    return interval > 0 and (iteration + 1) % interval == 0


def _periodic_free_overlap_buffers(iteration, model):
    if not _periodic_free_overlap_buffers_enabled(iteration):
        return
    freed = 0
    ...
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        print(
            f"[periodic-free-overlap-buffers] iter={iteration + 1} "
            f"interval={_phase_timing_int_env('MEGATRON_PERIODIC_FREE_OVERLAP_BUFFERS_INTERVAL', 0)} "
            f"model_chunks={freed}",
            flush=True,
        )


def _periodic_checkpoint_like_cleanup_enabled(iteration):
    if iteration is None:
        return False
    interval = _phase_timing_int_env("MEGATRON_PERIODIC_CHECKPOINT_LIKE_CLEANUP_INTERVAL", 0)
    return interval > 0 and (iteration + 1) % interval == 0


def _periodic_checkpoint_like_cleanup(iteration, model):
    if not _periodic_checkpoint_like_cleanup_enabled(iteration):
        return
    freed = 0
    for model_chunk in model:
        if hasattr(model_chunk, 'free_overlap_buffers'):
            ...
            freed += 1
    torch.cuda.empty_cache()
    ran_gc = _phase_timing_bool_env("MEGATRON_PERIODIC_CHECKPOINT_LIKE_CLEANUP_GC", False)
    if ran_gc:
        gc.collect()
    if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
        print(
            f"[periodic-checkpoint-like-cleanup] iter={iteration + 1} "
            f"interval={_phase_timing_int_env('MEGATRON_PERIODIC_CHECKPOINT_LIKE_CLEANUP_INTERVAL', 0)} "
            f"model_chunks={freed} gc={int(ran_gc)}",
            flush=True,
        )
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config, forward_backward_func, iteration=None):
    ...
    args = get_args()
    timers = get_timers()
    phase_timing = _phase_timing_enabled(iteration)
    phase_times = {}
    phase_start = _phase_timing_now(phase_timing)
    ...
    while rerun_state_machine.should_run_forward_backward(data_iterator):
        ...
        optimizer.zero_grad()
        phase_start = _phase_timing_record(phase_times, "zero_grad", phase_start, phase_timing)
        ...
        phase_start = _phase_timing_record(
            phase_times, "forward_backward", phase_start, phase_timing
        )
        if save_dgrads_in_this_iteration:
        ...
        phase_start = _phase_timing_record(
            phase_times, "reset_force_all_reduce", phase_start, phase_timing
        )
    ...
    if args.empty_unused_memory_level >= 1:
        torch.cuda.empty_cache()
    phase_start = _phase_timing_record(
        phase_times, "empty_unused_memory_l1", phase_start, phase_timing
    )
    ...
    # === GRADIENT EXPORT HOOK ===
    import os as _os
    if _os.environ.get("EXPORT_GRADS") == "1":
        import torch as _torch
        if _torch.distributed.get_rank() == 0:
            _grad_dict = {}
            _total_norm = 0.0
            for _name, _param in model[0].named_parameters():
                _g = None
                if hasattr(_param, "main_grad") and _param.main_grad is not None:
                    _g = _param.main_grad.float().cpu()
                elif _param.grad is not None:
                    _g = _param.grad.float().cpu()
                if _g is not None:
                    _grad_dict[_name] = _g
                    _total_norm += _g.norm(2).item() ** 2
            _total_norm = _total_norm ** 0.5
            _out = _os.environ.get("GRAD_EXPORT_PATH", "/tmp/megatron_grads.pt")
            _torch.save({"grads": _grad_dict, "grad_norm": _total_norm, "num_params": len(_grad_dict)}, _out)
            print(f"[GRAD EXPORT] Saved {len(_grad_dict)} grads to {_out}, norm={_total_norm:.6f}")
    # === END HOOK ===
    ...
    phase_start = _phase_timing_record(phase_times, "optimizer_step", phase_start, phase_timing)
    ...
    phase_start = _phase_timing_record(
        phase_times, "optimizer_sync_stats", phase_start, phase_timing
    )
    ...
    if update_successful:
        ...
        skipped_iter = 1
    phase_start = _phase_timing_record(phase_times, "scheduler", phase_start, phase_timing)

    # Empty unused memory.
    if args.empty_unused_memory_level >= 2:
        torch.cuda.empty_cache()
    phase_start = _phase_timing_record(
        phase_times, "empty_unused_memory_l2", phase_start, phase_timing
    )
    _periodic_empty_cache(iteration)
    phase_start = _phase_timing_record(
        phase_times, "periodic_empty_cache", phase_start, phase_timing
    )
    _periodic_free_overlap_buffers(iteration, model)
    phase_start = _phase_timing_record(
        phase_times, "periodic_free_overlap_buffers", phase_start, phase_timing
    )
    _periodic_checkpoint_like_cleanup(iteration, model)
    phase_start = _phase_timing_record(
        phase_times, "periodic_checkpoint_like_cleanup", phase_start, phase_timing
    )
    if phase_timing:
        phase_times["train_step_total"] = sum(phase_times.values())
        _phase_timing_report(iteration, phase_times)
...
def checkpoint_and_decide_exit(
    ...
    saved_checkpoint = checkpoint_already_saved
    ...
    if (
        ...
        and not checkpoint_already_saved
    ):
        save_checkpoint_and_time(
            iteration,
            model,
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        checkpoint_saved_before_eval = False
        if (
            args.save
            and args.save_interval
            and iteration % args.save_interval == 0
            ...
            print_rank_0(
                f"saving checkpoint before evaluation at iteration {iteration} "
                "because save_interval and eval_interval coincide"
            )
            save_checkpoint_and_time(
                iteration,
                model,
            ...
            checkpoint_saved_before_eval = True
        ...
        should_exit = checkpoint_and_decide_exit(
            ...
            checkpointing_context,
            train_data_iterator,
            checkpoint_already_saved=checkpoint_saved_before_eval,
        )
        if should_exit:
            break
...
def build_train_valid_test_data_loaders(build_train_valid_test_datasets_provider):
    ...
    if is_distributed or mpu.get_tensor_model_parallel_rank() == 0:
        ...
        if args.perform_rl_step:
            ...
            if args.full_validation or args.eval_iters > 0:
                valid_dataloaders = []
