...
_DATALOAD_TIMING_COUNT = 0
_DATALOAD_TIMING_TOTAL = 0.0
_DATALOAD_TIMING_MAX = 0.0
_DATALOAD_TIMING_WINDOW_COUNT = 0
_DATALOAD_TIMING_WINDOW_TOTAL = 0.0
_DATALOAD_TIMING_WINDOW_MAX = 0.0
_PACKED_BATCH_STATS_COUNT = 0


def _dataload_timing_enabled():
    return os.getenv("MEGATRON_DATALOAD_TIMING", "0") == "1"


def _record_dataload_next_time(elapsed):
    global _DATALOAD_TIMING_COUNT, _DATALOAD_TIMING_TOTAL, _DATALOAD_TIMING_MAX
    global _DATALOAD_TIMING_WINDOW_COUNT, _DATALOAD_TIMING_WINDOW_TOTAL
    global _DATALOAD_TIMING_WINDOW_MAX
    if not _dataload_timing_enabled():
        return
    _DATALOAD_TIMING_COUNT += 1
    _DATALOAD_TIMING_TOTAL += elapsed
    _DATALOAD_TIMING_MAX = max(_DATALOAD_TIMING_MAX, elapsed)
    _DATALOAD_TIMING_WINDOW_COUNT += 1
    _DATALOAD_TIMING_WINDOW_TOTAL += elapsed
    _DATALOAD_TIMING_WINDOW_MAX = max(_DATALOAD_TIMING_WINDOW_MAX, elapsed)

    interval = int(os.getenv("MEGATRON_DATALOAD_TIMING_INTERVAL", "800"))
    threshold_ms = float(os.getenv("MEGATRON_DATALOAD_TIMING_THRESHOLD_MS", "500"))
    rank = torch.distributed.get_rank() if torch.distributed.is_initialized() else 0
    if elapsed * 1000.0 >= threshold_ms:
        print(
            f"[dataload-next-slow] rank={rank} call={_DATALOAD_TIMING_COUNT} "
            f"wait_ms={elapsed * 1000.0:.3f}",
            flush=True,
        )
    if interval > 0 and _DATALOAD_TIMING_COUNT % interval == 0:
        avg_ms = 1000.0 * _DATALOAD_TIMING_WINDOW_TOTAL / max(1, _DATALOAD_TIMING_WINDOW_COUNT)
        max_ms = 1000.0 * _DATALOAD_TIMING_WINDOW_MAX
        total_avg_ms = 1000.0 * _DATALOAD_TIMING_TOTAL / max(1, _DATALOAD_TIMING_COUNT)
        print(
            f"[dataload-next] rank={rank} calls={_DATALOAD_TIMING_COUNT} "
            f"window_avg_ms={avg_ms:.3f} window_max_ms={max_ms:.3f} "
...
def _packed_batch_stats_enabled():
    return os.getenv("MEGATRON_PACKED_BATCH_STATS", "0") == "1"


def _record_packed_batch_stats(batch):
    global _PACKED_BATCH_STATS_COUNT
    if not _packed_batch_stats_enabled():
        return
    if not torch.distributed.is_initialized():
        return

    _PACKED_BATCH_STATS_COUNT += 1
    interval = int(os.getenv("MEGATRON_PACKED_BATCH_STATS_INTERVAL", "80"))
    if interval <= 0 or _PACKED_BATCH_STATS_COUNT % interval != 0:
        return

    cu_seqlens = batch.get("cu_seqlens")
    with torch.no_grad():
        dev = torch.cuda.current_device()
        has_cu = cu_seqlens is not None and cu_seqlens.numel() > 0
        if has_cu:
            cu = cu_seqlens.detach()
            if cu.dim() == 1:
                cu = cu.unsqueeze(0)
            lens = (cu[:, 1:] - cu[:, :-1]).reshape(-1).to(torch.float32)
        else:
            lens = torch.empty(0, dtype=torch.float32, device=dev)

        has_segments = lens.numel() > 0
        if has_segments:
            max_seqlen = batch.get("max_seqlen")
            if max_seqlen is None or max_seqlen.numel() == 0:
                local_max_seqlen = torch.max(lens)
            else:
                local_max_seqlen = max_seqlen.detach().reshape(-1).to(torch.float32).max()
            local_lens_sum = float(lens.sum().item())
            local_ge4096 = float((lens >= 4096).sum().item())
            local_ge8192 = float((lens >= 8192).sum().item())
            local_ge12000 = float((lens >= 12000).sum().item())
            local_max_segment = float(torch.max(lens).item())
            local_max_seqlen_value = float(local_max_seqlen.item())
        else:
            local_lens_sum = 0.0
            local_ge4096 = 0.0
            local_ge8192 = 0.0
            local_ge12000 = 0.0
            local_max_segment = 0.0
            local_max_seqlen_value = 0.0

        sums = torch.tensor(
            [
                1.0,
                1.0 if has_cu else 0.0,
                float(lens.numel()),
                local_lens_sum,
                local_ge4096,
                local_ge8192,
        ...
        maxes = torch.tensor(
            [local_max_segment, local_max_seqlen_value],
            device=dev,
            dtype=torch.float64,
        )
    ...
    if torch.distributed.get_rank() == 0:
        ranks = max(1.0, sums[0].item())
        has_cu_ranks = sums[1].item()
        segments = max(1.0, sums[2].item())
        print(
            "[packed-batch-stats] "
            f"call={_PACKED_BATCH_STATS_COUNT} "
            f"ranks={int(ranks)} "
            f"has_cu_ranks={int(has_cu_ranks)} "
            f"segments_per_rank={sums[2].item() / ranks:.2f} "
            f"mean_segment_len={sums[3].item() / segments:.2f} "
            f"max_segment_len={maxes[0].item():.0f} "
            f"max_seqlen={maxes[1].item():.0f} "
            f"frac_ge4096={sums[4].item() / segments:.4f} "
            f"frac_ge8192={sums[5].item() / segments:.4f} "
            f"frac_ge12000={sums[6].item() / segments:.4f}",
            flush=True,
        )
...
def get_batch_on_this_tp_rank(data_iterator, mtp_on_this_rank: bool = False):
    ...
    if (
        os.getenv("MEGATRON_DATALOAD_CP_BROADCAST", "0") == "1"
        ...
        def _broadcast_cu_seqlens_from_src(cu_seqlens):
            ...
            _broadcast_tc(buf)
            return buf if n > 0 else None

        def _broadcast_cu_seqlens_to_non_src():
            dev = torch.cuda.current_device()
            n_tensor = torch.empty((), dtype=torch.int64, device=dev)
            _broadcast_tc(n_tensor)
            n = int(n_tensor.item())
            max_cu_seqlens = args.seq_length + 1
            if n < 0 or n > max_cu_seqlens:
                warnings.warn(
                    f"Invalid cu_seqlens length received on tensor/context rank "
                    f"{group_rank}: {n}; treating this batch as non-packed."
                )
                n = 0
            if n == 0:
            ...
            _broadcast_tc(cu_seqlens)
            return cu_seqlens if n > 0 else None

        if group_rank == 0:
            assert data_iterator is not None
            if _dataload_timing_enabled():
                next_start = time.perf_counter()
                data = next(data_iterator)
                _record_dataload_next_time(time.perf_counter() - next_start)
            else:
                data = next(data_iterator)
            batch = {
            ...
            _broadcast_tc(batch['tokens'])
            _broadcast_tc(batch['labels'])
            _broadcast_tc(batch['loss_mask'])
            _broadcast_tc(batch['attention_mask'])
            _broadcast_tc(batch['position_ids'])
            batch['cu_seqlens'] = _broadcast_cu_seqlens_from_src(batch['cu_seqlens'])
            batch['cu_seqlens_padded'] = _broadcast_cu_seqlens_from_src(
                batch['cu_seqlens_padded']
            )
            _broadcast_tc(batch['max_seqlen'])
            _broadcast_tc(batch['local_cp_size'])
        else:
            shape = (args.micro_batch_size, args.seq_length)
            dev = torch.cuda.current_device()
            batch = {
                'tokens': torch.empty(shape, dtype=torch.int64, device=dev),
                'labels': torch.empty(shape, dtype=torch.int64, device=dev),
                'loss_mask': torch.empty(shape, dtype=torch.float32, device=dev),
                'attention_mask': (
                    torch.empty(
                ...
                'position_ids': torch.empty(shape, dtype=torch.int64, device=dev),
                'cu_seqlens': None,
                'cu_seqlens_padded': None,
                'max_seqlen': torch.empty(1, dtype=torch.int32, device=dev) if args.sft else None,
                'local_cp_size': None,
            }
            _broadcast_tc(batch['tokens'])
            _broadcast_tc(batch['labels'])
            ...
            _broadcast_tc(batch['attention_mask'])
            _broadcast_tc(batch['position_ids'])
            batch['cu_seqlens'] = _broadcast_cu_seqlens_to_non_src()
            batch['cu_seqlens_padded'] = _broadcast_cu_seqlens_to_non_src()
            _broadcast_tc(batch['max_seqlen'])
            _broadcast_tc(batch['local_cp_size'])

        _record_packed_batch_stats(batch)
        return batch
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        def _broadcast_cu_seqlens():
            ...
            if n < 0 or n > max_cu_seqlens:
                warnings.warn(
                    f"Invalid cu_seqlens length received on tensor-parallel rank "
                    f"{mpu.get_tensor_model_parallel_rank()}: {n}; treating this "
                    "batch as non-packed to keep collectives aligned."
                )
                n = 0
